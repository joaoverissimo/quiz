...
define('CACHE_DIR_NAME', '/jquerycms/upload/images/imagecache/');
define('CACHE_DIR', CURRENT_DIR . CACHE_DIR_NAME);
define('DOCUMENT_ROOT', $_SERVER['DOCUMENT_ROOT']);

// Images must be local files, so for convenience we strip the domain if it's there
//Obtem a imagem
$image = preg_replace('/^(s?f|ht)tps?:\/\/[^\/]+/i', '', (string) $_GET['img']);


$obj = new objJqueryimage($Conexao);
$obj->loadByCod($image);
if ($obj->getExiste()) {
    $image = "/jquerycms/upload/images/" . $obj->getValor();
} else {
    $image = "/jquerycms/upload/images/imagem-inexistente.png";
}

//More headers
// Define os headers para cache armazenar a imagem em cache do browser
header("Cache-Control: private, max-age=10800, pre-check=10800");
header("Pragma: private");
header("Expires: " . date(DATE_RFC822, strtotime(" 2 day")));

if (isset($_SERVER['HTTP_IF_MODIFIED_SINCE']) && (strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE']) == filemtime($obj->getFileName()))) {
    header('Last-Modified: ' . gmdate('D, d M Y H:i:s', filemtime($obj->getFileName())) . ' GMT', true, 304);
    exit;
}

header('Last-Modified: ' . gmdate('D, d M Y H:i:s', filemtime($obj->getFileName())) . ' GMT');

// Define o crop ratio
// Para redimensionar com crop
$CropRatio = "1:1";
