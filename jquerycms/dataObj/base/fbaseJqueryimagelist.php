...
// <editor-fold defaultstate="collapsed" desc="Load And Save">
    ...
    public function loadByArray($registro, $prefixArr = '') {
        ...
        if (isset($registro[$prefixArr . 'info'])) {
            $this->setInfo($registro[$prefixArr . 'info']); 
            $load = true;
        } 
    ...
    public function loadByCod($valor, $campo = 'cod') {
        $registro = dbJqueryimagelist::Carregar($this->Conexao, $valor, $campo);
    ...
    public function loadLeftByCod($valor, $campo = 'cod') {
        $registro = dbJqueryimagelist::CarregarLeft($this->Conexao, $valor, $campo);
    ...
    public function Save() {
        if (isset($this->cod) && is_numeric($this->cod)) {
            return dbJqueryimagelist::Update($this->Conexao, $this->getcod(),$this->getinfo());
        } else {    
            $this->cod = dbJqueryimagelist::Inserir($this->Conexao, $this->getinfo());
            return $this->cod != 0;
        }      
    }
    
    public function Delete() {
        return dbJqueryimagelist::Deletar($this->Conexao, $this->cod);
    }
...
// <editor-fold defaultstate="collapsed" desc="Stringuize">
    ...
    public function getHtmlTemplateString($html, $prefix = '') {
        ...
        if ($html == "") {
            $this->die = false;
            echo "\nMetodo Permitidos para objJqueryimagelist: [";
    ...
    public function getHtmlTemplateFile($file, $folder = "~/adm/jqueryimagelist/templates/") {
        if (!$file) {
...
// </editor-fold>
    ...
    public static function init($valor, $campo = 'cod', $die = true, $Conexao = null) {
        ...
        $obj = new objJqueryimagelist($Conexao, $die);
