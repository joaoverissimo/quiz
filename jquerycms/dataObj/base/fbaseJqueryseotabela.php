...
// <editor-fold defaultstate="collapsed" desc="Load And Save">
    ...
    public function loadByArray($registro, $prefixArr = '') {
        ...
        if (isset($registro[$prefixArr . 'tabela'])) {
            $this->setTabela($registro[$prefixArr . 'tabela']); 
            $load = true;
        } 
    ...
    public function loadByCod($valor, $campo = 'cod') {
        $registro = dbJqueryseotabela::Carregar($this->Conexao, $valor, $campo);
    ...
    public function loadLeftByCod($valor, $campo = 'cod') {
        $registro = dbJqueryseotabela::CarregarLeft($this->Conexao, $valor, $campo);
    ...
    public function Save() {
        if (isset($this->cod) && is_numeric($this->cod)) {
            return dbJqueryseotabela::Update($this->Conexao, $this->getcod(),$this->gettabela(),$this->gettitulo(),$this->getordem());
        } else {    
            $this->cod = dbJqueryseotabela::Inserir($this->Conexao, $this->gettabela(),$this->gettitulo(),$this->getordem());
            return $this->cod != 0;
        }      
    }
    
    public function Delete() {
        return dbJqueryseotabela::Deletar($this->Conexao, $this->cod);
    }
...
// <editor-fold defaultstate="collapsed" desc="Stringuize">
    ...
    public function getHtmlTemplateString($html, $prefix = '') {
        ...
        if ($html == "") {
            $this->die = false;
            echo "\nMetodo Permitidos para objJqueryseotabela: [";
    ...
    public function getHtmlTemplateFile($file, $folder = "~/adm/jqueryseotabela/templates/") {
        if (!$file) {
...
// </editor-fold>
    ...
    public static function init($valor, $campo = 'cod', $die = true, $Conexao = null) {
        ...
        $obj = new objJqueryseotabela($Conexao, $die);
